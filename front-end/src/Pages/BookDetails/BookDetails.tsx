import { Image, Breadcrumb, Divider, Descriptions } from 'antd';
    ...
    <>
      ...
      <div className='book-details-wrapper'>
        <div className='book-details-image'>
          <Image width={200} src={book?.formats['image/jpeg']} />
        </div>
        <div className='book-details-desc'>
          <Descriptions title={book?.title} bordered column={3}>
            <Descriptions.Item label='Authers'>
              {book?.authors
                .map(
                  (auth) =>
                    `${auth.name} (${auth.birth_year}-${auth.death_year})`
                )
                .join(', ')}
            </Descriptions.Item>
            <Descriptions.Item label='Languages'>
              {book?.languages.map((lang) => lang).join(', ')}
            </Descriptions.Item>
            <Descriptions.Item label='Copyright'>
              {book?.copyright ? 'YES' : 'NO'}
            </Descriptions.Item>
            <Descriptions.Item label='Bookshelves' span={2}>
              {book?.bookshelves.map((bs) => bs).join(', ')}
            </Descriptions.Item>
            <Descriptions.Item label='Download count'>
              {book?.download_count}
            </Descriptions.Item>
            <Descriptions.Item label='Subjects'>
              {book?.subjects.map((sub) => sub).join(', ')}
            </Descriptions.Item>
          </Descriptions>
        </div>
      </div>
