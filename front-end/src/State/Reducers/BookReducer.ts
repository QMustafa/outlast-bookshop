...
const reducer = (state = initialState, action: BookshopAction) : StateType => {
    switch(action.type) {
        ...
        case ActionType.BOOKS_FETCHED:
            const tempState = (!state.books || action.shouldReset) ? { ...state, isLoading: false, books: action.payload} :
            { ...state,
                isLoading: false,
                selectedBook: undefined,
                books: {...action.payload, results: state.books.results.concat(action.payload.results)}};
            return tempState;
        case ActionType.SELECT_BOOK:
            return { ...state, selectedBook: state.books!.results.find(b => b.id === action.payload)};
